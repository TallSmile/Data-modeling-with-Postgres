...
def process_song_file(cur, filepath):
    '''
    Processes provided song JSON file and inserts relevant
    ...
    artist_data = [list(row) for row in df.loc[:, ['artist_id',
                                                   'artist_name',
                                                   'artist_location',
                                                   'artist_latitude',
                                                   'artist_longitude'
...
def process_log_file(cur, filepath):
    ...
    time_df = pd.DataFrame(
        dict(zip(column_labels, time_data))).drop_duplicates()
    ...
    songplays_cols = ['start_time', 'user_id', 'level',
                      'song_id', 'artist_id', 'session_id', 'user_agent']
